################################################################
### Simulation code under violation of M3' (c): 
### additional M-U interaction in outcome model E[Y|A,M,C,U,W]
...
ybeta = c(intercept=0,M=1,A=1,W=-1,U=-1,c1=.2,c2=.2,MU=0.5) # Y=(1,M,A,W,U,C1,C2,MU)%*%ybeta
...
gen.data = function(N,param,DAG,addcov){
  ...
  Y = cbind(1,M,A,W,U,C1,C2,M*U)%*%param$ybeta + rnorm(N,mean=0,sd=1)
...
run.one = function(N,param,DAG,addcov){ 
  ...
  # (var.beta.Ma = coeftest(m_Ma.OLS, vcov = vcovHC(m_Ma.OLS, type = "HC1"))["A",2]^2)
  # X = (model.matrix(m_Ma.OLS))
  # Sigma = diag(m_Ma.OLS$residuals^2)
  ...
  (ols.NIE = beta.Ma*ols.beta.Ym)   #(true.val = param$mbeta["A"]*param$ybeta["M"])
  ## CI of naive OLS
  ...
  ###(2) ORACLE -- observe U W M
  if(DAG=="a"){
    m_OLS_oracle = lm(Y~M+A+C1+C2)
    m_Ma.OLS.oracle = lm(M~A+C1+C2)
  } else if(DAG=="b"){
    m_OLS_oracle = lm(Y~M*U+A+C1+C2)
    m_Ma.OLS.oracle = lm(M~A+U+C1+C2)
  } else if(DAG=="c"){
    m_OLS_oracle = lm(Y~M*U+W+A+C1+C2)
  ...
  if(DAG=="a"){
    (beta.Ym.oracle = m_OLS_oracle$coef["M"])##no violation
  }else{
    (beta.Ym.oracle = m_OLS_oracle$coef["M"]+m_OLS_oracle$coef["M:U"]*mean(U))##this is diff than no violation
  }
  
  (param$ybeta["M"]+param$ybeta["MU"]*mean(U)) ## true beta.Ym
  (NIE.oracle = beta.Ym.oracle*beta.Ma.oracle)
  (NIE.true = (param$ybeta["M"]+param$ybeta["MU"]*mean(U))*param$mbeta["A"])## true NIE
  ## CI of ORACLE
  ...
  #var.beta.Ym.oracle = (summary(m_OLS_oracle)$coef["M",2])^2
  ## diff from the no violation setting, here need to get var of (\theta_M+\theta_{MU}*E[W])
  p=length(which(!is.na(coef(m_OLS_oracle))))
  Ym.vec = c(0,1,rep(0,p-3),mean(U))
  var.beta.Ym.oracle=Ym.vec%*%sandwich(m_OLS_oracle)%*%Ym.vec
  #beta.Ym.oracle and var.beta.Ym.oracle are computed differentlhy than no vio 
  ...
  boot.func = function(data, indices,DAG){
    d = data[indices,] # allows boot to select sample 
    if(DAG=="a"){
      fit.theta_m = lm(Y~M+A+C1+C2, data=d)
      fit.beta_a = lm(M~A+C1+C2, data=d)
    } else if(DAG=="b"){
      fit.theta_m = lm(Y~M*U+A+C1+C2, data=d)
      fit.beta_a = lm(M~A+U+C1+C2, data=d)
    } else if(DAG=="c"){
      fit.theta_m = lm(Y~M*U+A+W+C1+C2, data=d)
    ...
    if(DAG=="a"){
      ...
      (NIE = (fit.theta_m$coef["M"]+fit.theta_m$coef["M:U"]*mean(U))*
         fit.beta_a$coef["A"])
    }
