################################################################
### Simulation code for the setting in the main manuscript
### Data generated under DAG (a)-(d)
...
ybeta = c(intercept=0,M=1,A=1,W=-1,U=-1,c1=.2,c2=.2) # Y=(1,M,A,W,U,C1,C2)%*%ybeta
...
gen.data = function(N,param,DAG,addcov){
  ### add two covariates: one continuous one binary
  if(addcov==T){
  ...
  Y = cbind(1,M,A,W,U,C1,C2)%*%param$ybeta + rnorm(N,mean=0,sd=1)
...
run.one = function(N,param,DAG,addcov){ 
  ...
  if(DAG=="a"){
    m_OLS_truth = lm(Y~M+A+W+C1+C2)
    m_Ma.OLS.oracle = lm(M~A+C1+C2)
  } else if(DAG=="b"){
    m_OLS_truth = lm(Y~M+A+U+C1+C2)
    m_Ma.OLS.oracle = lm(M~A+U+C1+C2)
  } else if(DAG=="c"){
    m_OLS_truth = lm(Y~M+A+U+W+C1+C2)
  ...
  (beta.Ym.oracle = m_OLS_truth$coef["M"])
  (NIE.oracle = beta.Ym.oracle*beta.Ma.oracle)
  (NIE.true = (param$ybeta["M"]*param$mbeta["A"])) ## true NIE
  ## CI of TRUTH
  ...
  var.beta.Ym.oracle = (summary(m_OLS_truth)$coef["M",2])^2
  var.oracle.NIE.robust = var.beta.Ma.oracle.robust*beta.Ym.oracle^2+var.beta.Ym.oracle*beta.Ma.oracle^2
  ci.oracle.robust = NIE.oracle + c(-1,1)*qnorm(1-0.05/2)*sqrt(var.oracle.NIE.robust)#no need /sqrt(n)
  var.oracle.NIE.nonrobust = var.beta.Ma.oracle.nonrobust*beta.Ym.oracle^2+var.beta.Ym.oracle*beta.Ma.oracle^2
  ci.oracle.nonrobust = NIE.oracle + c(-1,1)*qnorm(1-0.05/2)*sqrt(var.oracle.NIE.nonrobust)
  
  #####BOOT: now compute bootstrapped CI for OLS, genius, truth
  boot.type="perc"
